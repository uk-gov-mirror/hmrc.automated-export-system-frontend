...
import uk.gov.hmrc.automatedexportsystemfrontend.forms.amend.AmendDiscrepancyReferenceFormProvider
...
import uk.gov.hmrc.automatedexportsystemfrontend.views.html.amend.AmendDiscrepancyReferenceView

import scala.concurrent.Future

class AmendDiscrepancyReferenceControllerSpec extends SpecBase with MockitoSugar {

  def onwardRoute = Call("GET", "/foo")

  val submissionId = "12345"
  val formProvider = new AmendDiscrepancyReferenceFormProvider()
  val form: Form[String] = formProvider()

  lazy val amendDiscrepancyReferenceRoute: String = amendRoute.AmendDiscrepancyReferenceController.onPageLoad(NormalMode, submissionId).url

  "DiscrepancyReference Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, amendDiscrepancyReferenceRoute)

        val result = route(application, request).value

        val view = application.injector.instanceOf[AmendDiscrepancyReferenceView]

        status(result) shouldBe OK

        val body = contentAsString(result)
        body should include("What is the Declaration Goods Reference?")
        body should include("This is the 5-character reference shown on the item line in the export declaration.")
      }
    }

    "must populate the view correctly on a GET when the question has previously been answered" in {

      val userAnswers = UserAnswers(userAnswersId).set(AmendDiscrepancyReferencePage(submissionId), "answer").success.value
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, amendDiscrepancyReferenceRoute)
