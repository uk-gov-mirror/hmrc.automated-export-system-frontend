...
import uk.gov.hmrc.automatedexportsystemfrontend.forms.amend.AmendLocationTypeFormProvider
...
import uk.gov.hmrc.automatedexportsystemfrontend.views.html.amend.AmendLocationTypeView

import scala.concurrent.Future

class AmendLocationTypeControllerSpec extends SpecBase with MockitoSugar {

  def onwardRoute = Call("GET", "/foo")

  lazy val locationTypeRoute: String = amendRoute.AmendLocationTypeController.onPageLoad(NormalMode, submissionId).url

  val submissionId = "12345"
  val formProvider = new AmendLocationTypeFormProvider()
  ...
  "LocationType Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        ...
        val view = application.injector.instanceOf[AmendLocationTypeView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(AmendLocationTypePage(submissionId), LocationType.values.head).success.value
