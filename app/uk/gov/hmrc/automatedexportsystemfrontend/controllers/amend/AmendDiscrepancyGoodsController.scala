...
import uk.gov.hmrc.automatedexportsystemfrontend.models.{Mode, WhatHasChangedDetails}
...
class AmendDiscrepancyGoodsController @Inject() (
  ...
  formProvider: AmendDiscrepancyGoodsFormProvider,
  val controllerComponents: MessagesControllerComponents,
  view: AmendDiscrepancyGoodsView
  ...
  def onPageLoad(mode: Mode, submissionId: String): Action[AnyContent] = (actionBuilder andThen getData andThen requireData) { implicit request =>
    ...
    val preparedForm = request.userAnswers.get(AmendDiscrepancyGoodsPage(submissionId)) match {
      case None        => form
  ...
  def onSubmit(mode: Mode, submissionId: String): Action[AnyContent] = (actionBuilder andThen getData andThen requireData).async { implicit request =>
    form
      ...
      .fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(AmendDiscrepancyGoodsPage(submissionId), value))
            _ <- sessionRepository.set(updatedAnswers)
          } yield Redirect(amendNavigator.nextPage(AmendDiscrepancyGoodsPage(submissionId), mode, updatedAnswers))
      )
  }
}
